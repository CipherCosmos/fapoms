...
import { TransportRateService, TransportEstimate } from './transport-rate.service';
...
export class TransportRateController {
  ...
  /**
   * The dashboard's "try it" panel and any surface that wants per-mode costs for a journey.
   * Place accepts raw spellings — the service canonicalises exactly as the quote path does,
   * so what the estimator shows is what an offer would use.
   */
  @Get('estimate')
  @ApiOperation({ summary: 'Estimate per-mode journey costs for a distance and place' })
  async estimate(
