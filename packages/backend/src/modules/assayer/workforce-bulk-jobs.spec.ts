...
describe('WorkforceBulkJobsService', () => {
  it('accepts the run and returns a job id instead of doing the work', async () => {
    ...
    const service = new WorkforceBulkJobsService(queue as any);

    const result = await service.enqueueAppAccess(IDS, ACTOR);

    expect(result).toEqual({ jobId: '1', deduplicated: false });
    expect(queue.add).toHaveBeenCalledWith(
      WORKFORCE_BULK_JOB.APP_ACCESS,
      expect.objectContaining({ ids: ['a1', 'b2', 'c3'], requestedBy: 'hr-1', actor: ACTOR }),
...
describe('WorkforceBulkJobsWorker', () => {
  ...
  it("runs the batch inside the requester's scope, so it finds exactly the people they could", async () => {
    ...
    const worker = new WorkforceBulkJobsWorker(assayers as any);
