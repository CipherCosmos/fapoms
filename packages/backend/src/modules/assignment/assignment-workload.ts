 ...
 *
 * There used to be a sibling set, `DAY_EXCLUSIVE_ASSIGNMENT_STATUSES`, for the one-job-per-
 * assayer-per-day rule and its index `idx_assignments_single_active_assayer_day`. Owner decision
 * 2026-09-24: an assayer may hold several branches on one day, so that rule, its index (dropped by
 * migration 1800800000000) and the constant are gone. The BRANCH rule stays: one open job per
 * branch. `assignment-status-sets.spec.ts` pins this constant to its index definition.
 */
