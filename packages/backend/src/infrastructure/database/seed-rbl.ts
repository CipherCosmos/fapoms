...
// Map state names to dynamic Indian zone names
...
async function seed() {
  ...
  try {
    ...
    const clientConfigRepo = AppDataSource.getRepository(ClientConfigurationEntity);
    const zoneRepo = AppDataSource.getRepository(ZoneEntity);
    const branchRepo = AppDataSource.getRepository(BranchEntity);
    const assayerRepo = AppDataSource.getRepository(AssayerEntity);
    ...
    // 3. Clear existing details
    console.log('Cleaning existing database branches & assayers details...');
    await AppDataSource.query('TRUNCATE TABLE assignments, project_branches, branches, assayers, zones CASCADE;');

    // 4. Seed Zones dynamically for RBL client
    ...
    const zonesMap = new Map<string, ZoneEntity>();
    ...
    const branchesData = xlsx.utils.sheet_to_json(branchSheet) as any[];
    ...
    for (const row of branchesData) {
      ...
      // Determine dynamic zone, region, and territory
    ...
    const assayersData = xlsx.utils.sheet_to_json(assayerSheet) as any[];
