      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 300, overflowY: 'auto' }}>
        {client.lines.map((l) => (
          <label key={l.entryId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', cursor: l.onHold ? 'not-allowed' : 'pointer', opacity: l.onHold ? 0.6 : 1 }}>
            ...
            <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{fmtDate(l.serviceDate)}</span>
            <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{money(l.totalAmount)}</span>
          </label>
        ))}
      </div>
