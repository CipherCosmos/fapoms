              ...
              {filtered.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    onClick={() => toggle(a.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', cursor: 'pointer', borderRadius: 'var(--radius-sm)', background: isChecked ? 'var(--status-pending-bg)' : 'transparent' }}
                  >
