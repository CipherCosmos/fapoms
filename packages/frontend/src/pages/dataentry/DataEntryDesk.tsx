...
const DOC_LANES: { key: DocLane; label: string; hint: string; tone: string }[] = [
  { key: 'unallocated', label: 'Waiting to allocate', hint: 'Returned by the assayer, nobody working it yet', tone: '#fb923c' },
  { key: 'working', label: 'Being worked', hint: 'Delegated and in progress', tone: '#60a5fa' },
];
const CASE_LANES: { key: CaseLane; label: string; hint: string; tone: string; statuses: string[] }[] = [
  { key: 'review', label: 'In review', hint: 'With the head — approve, or send back with a note', tone: '#facc15', statuses: ['HUMAN_REVIEW', 'CORRECTION_REQUIRED'] },
  { key: 'approved', label: 'Approved', hint: 'Ready to submit to the client', tone: '#34d399', statuses: ['APPROVED'] },
  { key: 'submitted', label: 'Submitted', hint: 'Sent to the client', tone: '#22c55e', statuses: ['SUBMITTED'] },
];
    ...
    <div style={{ padding: '18px 22px', maxWidth: '1600px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '14px', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary,#9aa4b5)', fontSize: '13px', marginTop: '3px' }}>
      ...
      {err && (
        <div style={{ margin: '12px 0', padding: '9px 13px', borderRadius: '8px', background: 'rgba(239,68,68,0.1)', color: '#f87171', fontSize: '12.5px', display: 'flex', gap: '7px', alignItems: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', margin: '16px 0' }}>
        ...
        <Stat icon={<UserPlus size={15} />} value={queue?.unassigned ?? 0} caption="Waiting to allocate" tone={(queue?.unassigned ?? 0) > 0 ? '#fb923c' : undefined} />
        <Stat icon={<FileText size={15} />} value={casesByLane.review.length} caption="In review" tone={casesByLane.review.length ? '#facc15' : undefined} />
        <Stat icon={<CheckCircle2 size={15} />} value={casesByLane.approved.length} caption="Approved" tone={casesByLane.approved.length ? '#34d399' : undefined} />
        <Stat icon={<SubmitIcon size={15} />} value={casesByLane.submitted.length} caption="Submitted" />
        <Stat icon={<MessageSquare size={15} />} value={openClarifications} caption="Open clarifications" tone={openClarifications ? '#facc15' : undefined} />
      </div>
      ...
      {queue?.total === 0 && totalCases === 0 && (
        <div style={{ ...card, textAlign: 'center', padding: '40px' }}>
          ...
          <div style={{ fontSize: '12.5px', color: 'var(--text-muted,#7c8595)', marginTop: '4px' }}>
        ...
        {DOC_LANES.map((lane) => (
          <section key={lane.key} style={card}>
            ...
            {docsByLane[lane.key].length === 0 ? (
              ...
              return (
                <div key={d.id} style={{ padding: '10px 0', borderTop: '1px solid rgba(148,163,184,0.08)' }}>
                  ...
                  {d.assigneeName && (
                    <div style={{ fontSize: '11.5px', color: 'var(--text-secondary,#9aa4b5)', marginTop: '4px' }}>
                      with <strong>{d.assigneeName}</strong>{d.assignedAt && <span style={{ color: 'var(--text-muted,#7c8595)' }}> · {fmtWhen(d.assignedAt)}</span>}
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '6px', marginTop: '8px', flexWrap: 'wrap' }}>
                    {isHead && lane.key === 'unallocated' && (
                      <select defaultValue="" disabled={busy === d.id} onChange={(e) => assign(d.id, e.target.value)}
                        style={{ padding: '5px 8px', fontSize: '11.5px', borderRadius: '6px', background: 'var(--bg-page,#0d1016)', color: 'inherit', border: '1px solid var(--border-color,#232833)' }}>
        ...
        {CASE_LANES.map((lane) => (
          <section key={lane.key} style={card}>
            ...
            {casesByLane[lane.key].length === 0 ? (
              ...
              return (
                <div key={c.id} style={{ padding: '10px 0', borderTop: '1px solid rgba(148,163,184,0.08)' }}>
...
const LaneHeader: React.FC<{ tone: string; label: string; hint: string; count: number }> = ({ tone, label: text, hint, count }) => (
  <>
    ...
    <div style={{ fontSize: '11px', color: 'var(--text-muted,#7c8595)', marginBottom: '10px' }}>{hint}</div>
  </>
);

const Empty: React.FC = () => <div style={{ fontSize: '12px', color: 'var(--text-muted,#7c8595)', padding: '14px 0' }}>Empty.</div>;
...
const Stat: React.FC<{ icon: React.ReactNode; value: React.ReactNode; caption: string; tone?: string }> = ({ icon, value, caption, tone }) => (
  <div style={{ ...card, flex: '1 1 150px', minWidth: 0 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '7px', color: tone ?? 'var(--text-primary,#e6e9ef)' }}>
