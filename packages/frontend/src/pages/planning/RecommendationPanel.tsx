  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', overflow: 'hidden', ...(flex ? { flex: 1, minWidth: 0 } : { width: `${width}px`, minWidth: `${width}px` }) }}>
      {selectedPb ? (
        isDone ? (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontSize: 'var(--text-2xl)' }}>{selectedPb.status === 'CLOSED' ? '✅' : '🔍'}</span>
              <div>
                <div style={{ fontSize: 'var(--text-md)', fontWeight: 800, color: selectedPb.status === 'CLOSED' ? 'var(--success)' : 'var(--warning)' }}>
            ...
            {selectedPb.assignment && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', background: 'var(--bg-surface-2)', borderRadius: '8px', border: '1px solid var(--border-hair)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 'var(--text-3xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '2px' }}>ASSAYER</div>
                    <div style={{ fontSize: 'var(--text-sm)', color: 'var(--accent)', fontWeight: 600 }}>👤 {selectedPb.assignment.assayer?.displayName}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 'var(--text-3xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '2px' }}>AGREED FEE</div>
                    <div style={{ fontSize: 'var(--text-sm)', color: 'var(--success)', fontWeight: 700 }}>{assignmentFee(selectedPb.assignment)}</div>
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 'var(--text-3xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '2px' }}>BRANCH STATUS</div>
                  <span style={{ fontSize: 'var(--text-xs)', padding: '2px 8px', borderRadius: '4px', background: selectedPb.status === 'CLOSED' ? 'var(--status-active-bg)' : 'var(--status-pending-bg)', color: selectedPb.status === 'CLOSED' ? 'var(--success)' : 'var(--warning)', fontWeight: 700 }}>
                    {branchStatusLabel(selectedPb.status)}
                  </span>
                </div>
              </div>
            )}
            <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)', fontStyle: 'italic', lineHeight: 1.5 }}>
          ...
          <>
            <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-surface-2)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <span style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 700 }}>MATCHING INSPECTOR</span>
                  <div style={{ fontSize: 'var(--text-md)', fontWeight: 800, color: 'var(--text-primary)', marginTop: '1px' }}>{selectedPb.branch.name}</div>
                </div>
                <button onClick={onRefresh}
                  className="btn btn-secondary" title="Refresh candidates"
                  style={{ padding: '4px 8px', fontSize: 'var(--text-2xs)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {planDate != null && onPlanDateChange && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', userSelect: 'none' }}
                    ...
                    <input
                      ...
                      style={{ fontSize: 'var(--text-3xs)', padding: '2px 6px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', color: 'var(--accent)', fontWeight: 600, outline: 'none' }}
                    />
                  </label>
                )}
                  ...
                  <>
                    ...
                    {slaEnabled && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                        <input
                          ...
                          style={{ width: '56px', fontSize: 'var(--text-3xs)', padding: '2px 5px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', color: 'var(--warning)', outline: 'none' }}
                        />
                        <span style={{ fontSize: 'var(--text-3xs)', color: 'var(--warning)' }}>km</span>
                      </span>
                    )}
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }} title="Exact distance limit, when the menu's options do not fit">
                      <input
                        ...
                        style={{ width: '60px', fontSize: 'var(--text-3xs)', padding: '2px 5px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', color: 'var(--accent)', outline: 'none' }}
                      />
                      <span style={{ fontSize: 'var(--text-3xs)', color: 'var(--accent)' }}>km</span>
                    </span>
                  </>
                ) : (
                  <details style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}>
        ...
        */
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '10px', padding: '24px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 'var(--text-sm)' }}>
          <div style={{ fontSize: 'var(--text-md)', fontWeight: 700, color: 'var(--text-primary)' }}>Ready to staff a branch</div>
          {onNextUnassigned && (
            <button
              type="button"
              onClick={onNextUnassigned}
              className="btn btn-primary"
              style={{ padding: '8px 16px', fontSize: 'var(--text-sm)', fontWeight: 700 }}
          ...
          {nextBranchName && (
            <div style={{ fontSize: 'var(--text-2xs)' }}>Next up: <b style={{ color: 'var(--text-secondary)' }}>{nextBranchName}</b></div>
          )}
          <div style={{ fontSize: 'var(--text-2xs)', maxWidth: '300px', lineHeight: 1.5 }}>
