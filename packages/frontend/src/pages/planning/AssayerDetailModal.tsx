...
const StatTile: React.FC<{ icon: React.ReactNode; caption: string; value: React.ReactNode; valueText: string; tone?: string }> = ({ icon, caption, value, valueText, tone }) => (
  <div role="group" aria-label={`${caption}: ${valueText}`} className="glass-card" style={{ padding: '12px', borderRadius: 'var(--radius-md)' }}>
    <div aria-hidden="true" style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '2px', display: 'flex', alignItems: 'center', gap: '3px' }}>{icon} {caption}</div>
    <div aria-hidden="true" style={{ fontSize: 'var(--text-xl)', fontWeight: 700, color: tone ?? 'var(--accent-primary)' }}>{value}</div>
  </div>
);
          ...
          <>
            ...
            <div className="glass-card" style={{ padding: '20px', borderRadius: 'var(--radius-md)', margin: '0 4px 12px 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--on-accent)', fontSize: 'var(--text-lg)', fontWeight: 700, flexShrink: 0 }}>
                    {profile.displayName.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, margin: 0 }}>{profile.displayName}</h3>
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '2px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>{profile.assayerCode}</span>
                      <span style={{ width: '3px', height: '3px', borderRadius: '50%', background: 'var(--text-muted)' }} />
                      <span style={{ fontSize: 'var(--text-2xs)', padding: '1px 6px', borderRadius: '8px', background: profile.lifecycleStatus === 'ACTIVE' ? 'var(--status-active-bg)' : 'var(--status-pending-bg)', color: profile.lifecycleStatus === 'ACTIVE' ? 'var(--status-active)' : 'var(--warning)', fontWeight: 500 }}>{profile.lifecycleStatus}</span>
                      <span style={{ width: '3px', height: '3px', borderRadius: '50%', background: 'var(--text-muted)' }} />
                      <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '3px' }}><Briefcase size={10} /> {profile.employmentType}</span>
                      <span style={{ width: '3px', height: '3px', borderRadius: '50%', background: 'var(--text-muted)' }} />
                      <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '3px' }}><Star size={10} /> {profile.experienceYears} yrs exp</span>
                      {profile.joiningDate && (
                        <>
                          <span style={{ width: '3px', height: '3px', borderRadius: '50%', background: 'var(--text-muted)' }} />
                          <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}>Joined {new Date(profile.joiningDate).toLocaleDateString()}</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 700, color: profile.averageRating >= 4 ? 'var(--status-active)' : profile.averageRating >= 3 ? 'var(--warning)' : 'var(--danger)' }}>
              ...
              <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid var(--border-color)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><MapPin size={11} /> {[profile.address, profile.city, profile.district, profile.state, profile.pincode].filter(Boolean).join(', ') || `${profile.city}, ${profile.state}`}</div>
                {profile.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><Phone size={11} /> {profile.phone}</div>}
                {profile.email && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><Mail size={11} /> {profile.email}</div>}
                {profile.department && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><Briefcase size={11} /> {profile.department}</div>}
                {profile.region && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><MapPin size={11} /> Region: {profile.region}</div>}
                {profile.languages && profile.languages.length > 0 && <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)' }}><Globe size={11} /> {profile.languages.join(', ')}</div>}
              </div>
              {profile.specializations && profile.specializations.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px', marginTop: '8px' }}>
                  {profile.specializations.map(s => <span key={s} style={{ padding: '2px 6px', background: 'var(--bg-surface-2)', color: 'var(--text-secondary)', borderRadius: '8px', fontSize: 'var(--text-3xs)' }}>{s}</span>)}
                </div>
              )}
              {(profile as any).locationNeedsConfirmation && (
                <div style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '5px', fontSize: 'var(--text-3xs)', fontWeight: 600, color: 'var(--warning)' }}>
                ...
                return (
                  <button key={t.key} id={`adm-tab-${t.key}`} role="tab" aria-selected={on} aria-controls={`adm-panel-${t.key}`}
                    ...
                    {t.key === 'qualification' && snapshot && snapshot.riskFlags.length > 0 && (
                      <span aria-label={`${snapshot.riskFlags.length} open flags`} style={{ fontSize: 'var(--text-3xs)', fontWeight: 700, padding: '0px 5px', borderRadius: '8px', background: 'var(--status-cancelled-bg)', color: 'var(--danger)' }}>{snapshot.riskFlags.length}</span>
                    )}
                  </button>
                );
              })}
            ...
            <div id={`adm-panel-${tab}`} role="tabpanel" aria-labelledby={`adm-tab-${tab}`} tabIndex={0} style={{ padding: '14px 4px 4px 2px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <>
                  {candidate && (
                    <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-hair)' }}>
                      <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 8px', display: 'flex', alignItems: 'center', gap: '5px' }}><Compass size={13} /> This match{branchName ? ` — ${branchName}` : ''}</h4>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                      ...
                      {candidate.dateConflict && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: 'var(--text-3xs)', fontWeight: 600, padding: '4px 8px', borderRadius: 'var(--radius-sm)', background: 'var(--status-pending-bg)', color: 'var(--warning)', marginBottom: '8px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' }}>
                    ...
                    <StatTile icon={<CheckCircle2 size={10} />} caption="Completed" value={<>{effectiveCompleted}<span style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 500 }}> ({completionRate}%)</span></>} valueText={`${effectiveCompleted}, ${completionRate} percent`} tone="var(--status-active)" />
                    ...
                    <a href={`/billing/statement?assayer=${profile.id}`} className="glass-card" style={{ padding: '12px', borderRadius: 'var(--radius-md)', textDecoration: 'none', color: 'inherit' }} title="Open the assayer's statement — earned, paid, owed">
                      <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '2px', display: 'flex', alignItems: 'center', gap: '3px' }}><DollarSign size={10} /> Earnings</div>
                      <div style={{ fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--accent)' }}>Statement →</div>
                    </a>
                  </div>

                  {profile.notes && (
                    <div className="glass-card" style={{ padding: '12px', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                      <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '4px' }}>NOTES</div>
                      {profile.notes}
                    </div>
                  )}
                </>
              ...
              {tab === 'qualification' && (
                <>
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><ShieldCheck size={13} /> Qualification score</h4>
                    {loadingExtra ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Loading…</div>
                    ) : !qualification ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Could not load the qualification profile.</div>
                    ) : (
                      <>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '10px' }}>
                          <span style={{ fontSize: 'var(--text-3xl)', fontWeight: 800, color: scoreTone(qualification.overall.effective), fontVariantNumeric: 'tabular-nums' }}>
                            {qualification.overall.effective ?? '—'}
                          </span>
                          <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>/ 100 overall</span>
                          {qualification.overall.override && <span style={{ fontSize: 'var(--text-3xs)', fontWeight: 700, color: 'var(--warning)', textTransform: 'uppercase' }}>adjusted</span>}
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          {qualification.dimensions.map((d) => (
                            <div key={d.key} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                              ...
                              {d.basis.length > 0 && (
                                <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', lineHeight: 1.5 }}>{d.basis.join(' · ')}</div>
                              )}
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)', border: clientId && partnerForClient && !plannableForClient ? '1px solid var(--danger)' : undefined }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><ShieldAlert size={13} /> Eligibility for this client</h4>
                    {!clientId ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Open this candidate from a branch to see client-specific eligibility.</div>
                    ) : loadingExtra ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Loading…</div>
                    ) : !partnerForClient ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>No empanelment record for this client yet.</div>
                    ) : (
                      <>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 'var(--text-2xs)', fontWeight: 700, padding: '3px 8px', borderRadius: '8px', background: partnerForClient.barred ? 'var(--status-cancelled-bg)' : plannableForClient ? 'var(--status-active-bg)' : 'var(--status-pending-bg)', color: partnerForClient.barred ? 'var(--danger)' : plannableForClient ? 'var(--success)' : 'var(--warning)' }}>
                          ...
                          <span style={{ fontSize: 'var(--text-lg)', fontWeight: 800, color: scoreTone(partnerForClient.effective), fontVariantNumeric: 'tabular-nums' }}>{partnerForClient.effective ?? '—'}</span>
                          <span style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)' }}>for {partnerForClient.client.name}</span>
                        </div>
                        {partnerForClient.standingCap != null && (
                          <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--warning)', marginBottom: '6px' }}>Score capped at {partnerForClient.standingCap} — {partnerForClient.standingReason || 'empanelment standing'}.</div>
                        )}
                        {partnerForClient.gaps.length > 0 && (
                          <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-secondary)' }}>
                  ...
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><Clock size={13} /> Current workload</h4>
                    {loadingExtra ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Loading…</div>
                    ) : !snapshot ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Could not load live workload.</div>
                    ) : (
                      <>
                        <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                  ...
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><AlertTriangle size={13} /> Data-integrity flags</h4>
                    {loadingExtra ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Loading…</div>
                    ) : !snapshot ? (
                      ...
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>Could not load the data-integrity flags, so this is not saying there are none.</div>
                    ) : snapshot.riskFlags.length === 0 ? (
                      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--success)', display: 'flex', alignItems: 'center', gap: '5px' }}><CheckCircle2 size={12} /> No open flags on this record.</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {snapshot.riskFlags.map((f, i) => (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', padding: '6px 8px', background: 'var(--status-cancelled-bg)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-3xs)' }}>
                  ...
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><Award size={13} /> Skills & Certifications</h4>
                    <div style={{ marginBottom: '10px' }}>
                      <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '4px' }}>SKILLS</div>
                      {profile.skills && profile.skills.length > 0 ? (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px' }}>
                          {profile.skills.map(s => <span key={s} style={{ padding: '2px 6px', background: 'rgba(216,174,71,0.1)', color: 'var(--accent-primary)', borderRadius: '8px', fontSize: 'var(--text-3xs)' }}>{s}</span>)}
                        </div>
                      ) : <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>No skills recorded</div>}
                    </div>
                    <div>
                      <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '4px' }}>CERTIFICATIONS</div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {profile.certifications.map(c => (
                            <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 6px', background: 'var(--status-active-bg)', borderRadius: 'var(--radius-sm)' }}>
                              <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-primary)' }}>{c.name}</span>
                              <span style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)' }}>Exp: {new Date(c.expiryDate).toLocaleDateString()}</span>
                            </div>
                          ))}
                        </div>
                      ) : <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--text-muted)' }}>No certifications recorded</div>}
                    </div>
                  </div>
                </>
              )}
              ...
              {tab === 'history' && (
                <>
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><TrendingUp size={13} /> Performance Insights</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div>
                        <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '3px' }}>COMPLETION RATE</div>
                        <div style={{ height: '6px', background: 'var(--bg-primary)', borderRadius: '3px', overflow: 'hidden' }}>
                        ...
                        <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-secondary)', marginTop: '1px' }}>{completionRate}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '3px' }}>ON-TIME DELIVERY</div>
                        <div style={{ height: '6px', background: 'var(--bg-primary)', borderRadius: '3px', overflow: 'hidden' }}>
                        ...
                        <div style={{ fontSize: 'var(--text-3xs)', color: 'var(--text-secondary)', marginTop: '1px' }}>{onTimeRate}%</div>
                      </div>
                      <div style={{ display: 'flex', gap: '14px', fontSize: 'var(--text-2xs)', color: 'var(--text-secondary)', marginTop: '2px' }}>
                  ...
                  <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                    <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><Layers size={13} /> Audit History & Fee Logs</h4>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '260px', overflowY: 'auto' }}>
                        {profile.auditHistory.map(ah => (
                          <div key={ah.id} style={{ padding: '8px 10px', background: 'var(--bg-primary)', borderRadius: 'var(--radius-sm)', borderLeft: '3px solid var(--accent)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-primary)' }}>{ah.branch_name || 'Branch Audit'}</div>
                    ...
                    ) : <div style={{ textAlign: 'center', padding: '12px', color: 'var(--text-muted)', fontSize: 'var(--text-2xs)' }}>No audit history recorded.</div>}
                  </div>
                </>
              )}
              ...
              {tab === 'remarks' && (
                <div className="glass-card" style={{ padding: '14px', borderRadius: 'var(--radius-md)' }}>
                  <h4 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: '5px' }}><Star size={13} /> Staff remarks</h4>
