...
| 3.10 | **Malware scanning — needs TWO files, read together. EICAR alone returns a false pass.** Upload a **harmless control file** (any small clean PDF) and the **EICAR** string to the same path, e.g. `POST /feedback/attachments`. | The control file is the discriminating one. `FileScanService.scanBuffer` catches the EICAR signature **in process, before it ever looks at `CLAMAV_HOST`** — deliberately, so the wiring is testable without a scanner — so a rejected EICAR proves the in-process guard fired and proves **nothing** about whether clamd is reachable. A deployment with no scanner at all rejects EICAR exactly the same way (`file-scan.service.spec.ts`, "EICAR is rejected with NO scanner configured"). Read them as a pair: **control → 503** means no reachable scanner (a FAIL, however the EICAR went); **control → 2xx on a path known to fail closed** means the bytes really reached clamd and came back clean; **EICAR → 400 `UPLOAD_REJECTED`** then shows the path refuses what the scanner finds. Both together are conclusive; either alone is not. **The fail-closed condition is not optional**: the control only discriminates because `FILE_SCAN_REQUIRED=true` makes an unscannable file 503. With it unset, a deployment with no scanner answers 2xx as well and the control proves nothing — so establish `FILE_SCAN_REQUIRED=true` first (in production `main.ts` refuses to boot without it, so a serving API is itself the proof). `scripts/acceptance/verify-deployment.mjs` runs this as 3.10 with `AC_EICAR=1`, and reports UNKNOWN rather than PASS when it cannot establish the fail-closed condition. Note the image publishes no arm64 build. |
