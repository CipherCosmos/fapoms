...
- **External-call resilience:** S3/MinIO and OSRM routing calls have **no timeouts** — a hung dependency stalls the event loop; the production template sets `ROUTING_PROVIDER=google` but **no Google provider exists**, so production silently falls back to straight-line distances and **under-pays assayers**. _(Resolved 2026-08-17, not by adding Google: OSRM is now the default provider with a hard timeout, a circuit breaker, `/table` batching and a 30-day Redis route cache; every figure carries `source: OSRM | ESTIMATE`; an unknown `ROUTING_PROVIDER` value warns at boot and uses OSRM; the template now says `OSRM`. See `packages/backend/src/modules/geo/routing.provider.ts`.)_
...
10. **External-call timeouts** (S3, OSRM) + the missing **Google routing provider** (assayers are being under-paid). _(medium)_ _(Done 2026-08-17 — road routing via OSRM by default rather than a Google provider; see §6 "External-call resilience".)_
